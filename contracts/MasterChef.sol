...
interface IMigratorChef {
    // Perform LP token migration from legacy Sugar to SugarSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Sugar LP tokens.
    // SugarSwap must mint EXACTLY the same amount of SugarSwap LP tokens or
    // else something bad will happen. Traditional Sugar does not
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        sugar.mint(address(syrup), sugarReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit Sugar by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw Sugar by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake Sugar tokens to MasterChef
    ...
    // Withdraw Sugar tokens from STAKING.
    ...
    function safeSugarTransfer(address _to, uint256 _amount) internal {
        syrup.safeSugarTransfer(_to, _amount);
    }
